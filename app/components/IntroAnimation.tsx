...
const LETTERS_FIRST  = ['A', 'S', 'J', 'I', 'A', 'D']
const LETTERS_SECOND = ['F', 'A', 'R', 'O', 'O', 'Q']
    ...
    <div className={`intro-overlay ${phase === 'exit' ? 'intro-exit' : ''}`} aria-hidden="true">
      ...
      <div className="intro-content">
        <div className="intro-stamp">
          ...
          <span className="intro-stamp-text">Est. 2012 · Riyadh & Worldwide</span>
          <div className="intro-stamp-line" />
        </div>

        <div className={`intro-brand-primary-row ${fontClass}`}>
          {LETTERS_FIRST.map((l, i) => (
            <span key={`${l}-${i}`} className="intro-letter-wrap" style={{ animationDelay: `${0.12 + i * 0.08}s` }}>
        ...
        <div className={`intro-brand-secondary-row ${fontClass}`}>
          {LETTERS_SECOND.map((l, i) => (
            <span key={i} className="intro-brand-secondary-letter" style={{ animationDelay: `${0.6 + i * 0.055}s` }}>
              {l}
            </span>
          ))}
        </div>
